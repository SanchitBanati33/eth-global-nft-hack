...
contract ZeusNFT is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant zeusSupply = 10000;
    uint256 public zeusPrice = 0.0001 ether; // 100000000000000 wei
    ...
    constructor(string memory baseURI) ERC721("ZeusNFT", "ZEUS")  {
        setBaseURI(baseURI);
    }

    function mintZeus(uint256 numberOfTokens, address account) public payable {
        ...
        require( supply + numberOfTokens <= (zeusSupply - reserve), "Purchase exceeds maximum supply of Zeus tokens" );
        require( msg.value >= zeusPrice * numberOfTokens,  "Ether sent is not correct" );

        for(uint256 i = 1; i <= numberOfTokens; i++){
            _safeMint(account, supply + i );
        }
    }
    function reserveZeus(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= reserve, "Exceeds reserved supply" );
        uint256 supply = totalSupply();
    ...
    function claimZeus(uint numberOfTokens) public {
        uint256 supply = totalSupply();
        require(numberOfTokens <= 4, "You can mint max 4 tokens at a time");
        require(totalSupply() + numberOfTokens <= (zeusSupply - reserve), "Exceeds max supply");
    ...
    function setName(uint256 _tokenId, string memory _name)
        ...
        returns (string memory)
    {
        require(ownerOf(_tokenId) == msg.sender, "Only zeus owner can name it");
